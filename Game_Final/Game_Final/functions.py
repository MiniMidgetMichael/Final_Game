...
def turtle_good_input(prompt, screen, typ=None, values=None):
        assert (type(prompt) is str),"prompt must be <class 'str'>, not %s" % type(prompt)
        assert (not(typ is None) or not(values is None)),"must be one of either 'typ' or 'values'"
        assert (type(screen) is turtle._Screen),"screen must be <class 'turtle._Screen'>, not %s" % type(screen)
        if not(typ is None):
            assert (isclass(typ)),"typ must be a class type"
        if not(values is None):
            assert (type(values) is list),"values must be <class 'list'>, not %s" % type(values)
        ...
        is_good = False
        while not(is_good):
            ans = screen.textinput(prompt, prompt).casefold()
            if not(typ is None):
                try:
                    typ(ans)
                    is_good = True
